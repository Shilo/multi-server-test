...
A master-owned SQLite persistence MVP now exists (guests, name-only login, saved world/position); see [VirtuCade Database MVP](virtucade-database-mvp.md). There is still no gateway process, standalone chat process, real auth server, Docker layer, or external fleet service in this refactor.
...
A master-owned SQLite MVP is now implemented (see [VirtuCade Database MVP](virtucade-database-mvp.md)): the master is the single DB writer, identity rides the world join ticket into spawn data, and worlds report positions back to the master. The broader intended ownership split, as more durable data is added, is:
...
- Login is name-only with no password (see [VirtuCade Database MVP](virtucade-database-mvp.md)); no real auth.
- Persistence is the SQLite MVP only (accounts + last world/position); no inventory/currency yet.
