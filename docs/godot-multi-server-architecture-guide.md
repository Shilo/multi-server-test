...
- `world_server`: temporary gameplay server process, one instance per active world key.
...
Direct launches without a launch token are useful for isolated world-scene debugging, but they do not register with master. If no user argument is provided, the world server starts `hub`. If the key is invalid, startup fails.
