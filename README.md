...
Minimum guardrails already present: master-owned process launch, per-launch world registration tokens, heartbeat expiry, player-count idle shutdown, target-world startup checks for transfer approval, and chat length/rate caps. Missing before public testing: authenticated sessions, server-side portal/travel authority, persistence, remote host configuration, systemd/service hardening for the master, and stronger movement validation.
