...
World registration uses per-launch tokens generated by `server/master/world_process_manager.gd`, not a shared secret stored in client-exported scripts. Client world joins use short-lived, one-use join tickets issued by master only after the client has confirmed the target world's assets are available, then forwarded to the target world before the client is allowed to spawn. The master can advertise world routes and pack metadata without starting the world process; the world process starts when the client requests its join after PackRat is finished.
...
Minimum guardrails already present: master-owned process launch, per-launch world registration tokens, ACK-gated registration, short-lived world join tickets, server-side portal/travel validation, start-timeout reaping, heartbeat expiry, player-count plus pending-join idle shutdown, target-world startup checks for transfer approval, chat length/rate caps, and master-only database access with server-validated position saves. Missing before public testing: authenticated sessions (login is name-only today), remote host configuration, replacing command-line launch tokens on shared hosts, systemd/service hardening for the master, and stronger movement validation.
