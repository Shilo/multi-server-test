...
- Local-first networking defaults with environment/query overrides for hosted tests.
...
- `editor/run_instance_grid.gd`: editor-only autoload that tiles visible Run Instances windows for manual debugging. Client/server exports include this tiny script so the project autoload remains valid, but it no-ops outside visible editor runs.
...
This MVP keeps local bind ports in `shared/net/net_config.gd`. Playable worlds live at `server/worlds/<world_key>/<world_key>.tscn`, discovered by folder convention. Ports are derived from the sorted world keys. Servers use Godot's default `create_server(port)` bind behavior, while clients dial advertised URLs from environment variables, Web query parameters, or the local defaults.
...
```text
MULTI_SERVER_MASTER_URL=<full ws:// or wss:// master URL override>
MULTI_SERVER_MASTER_HOST=127.0.0.1
MULTI_SERVER_MASTER_SCHEME=ws
MULTI_SERVER_WORLD_URL_TEMPLATE=<optional full world URL template with {world} and/or {port}>
MULTI_SERVER_WORLD_HOST=127.0.0.1
MULTI_SERVER_WORLD_SCHEME=ws
...
server-side filesystem directory where the current PCK files live. Web exports
can also receive the same public routing through query params such as
`?master_host=game.example.com&world_host=game.example.com&master_scheme=wss&world_scheme=wss`.
For reverse-proxy layouts where worlds are not exposed as raw derived ports,
prefer `MULTI_SERVER_WORLD_URL_TEMPLATE`, for example
`wss://game.example.com/worlds/{world}` or `wss://game.example.com:{port}`.
The equivalent Web query parameter is `world_url_template`.
