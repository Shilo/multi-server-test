...
For this test, the Web client still points at `127.0.0.1` / `ws` by default, so
...
This prototype assumes master/world server processes run together on one host
and use local ports internally.
