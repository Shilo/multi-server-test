...
```powershell
powershell -ExecutionPolicy Bypass -File tools\export_world_pack.ps1
powershell -ExecutionPolicy Bypass -File tools\export_all.ps1
powershell -ExecutionPolicy Bypass -File tools\run_smoke.ps1 -UseExported -InitialOnly
```

`-InitialOnly` is intentional for exported smoke until every world has a
downloadable pack. Editor/headless smoke still runs the full portal sequence
...
Export the current hub world pack:
...
The export scripts use two Windows Desktop executable presets plus one hub world pack preset:

- `Windows Client`: no role feature tag.
- `Windows Server`: dedicated server export with the `server` feature tag.
- `Hub World Pack`: exports `res://server/worlds/hub/hub.tscn` as a standalone client-downloadable pack.
...
This MVP keeps the advertised URL host in `shared/net/net_config.gd`. Playable worlds live at `server/worlds/<world_key>/<world_key>.tscn`; the manifest at `server/worlds/world_manifest.json` can add display names and downloadable pack metadata. Ports are still derived from the sorted world keys. Servers use Godot's default `create_server(port)` bind behavior, while clients and world servers dial URLs built from `HOST`.
